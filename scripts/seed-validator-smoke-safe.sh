...
mkdir -p "$SWIFTPM_BUILD_DIR" "$SWIFTPM_CACHE_DIR" "$SWIFTPM_CONFIG_DIR" "$SWIFTPM_SECURITY_DIR" "$HOME" "$XDG_CACHE_HOME" "$CLANG_MODULE_CACHE_PATH" "$SWIFT_MODULE_CACHE_PATH" Tools/benchmarks

if [ ! -f "Package.swift" ]; then
  echo "Package.swift not found yet. Nothing to validate." >&2
  exit 0
fi
...
/usr/bin/xcrun swift run \
  ...
  --chunk-size 16 \
  --vertical-scale 8 \
  --report Tools/benchmarks/seed_validator_smoke.json

echo "Seed validator smoke OK"
